...
void initialise_Prog(Prog *p, FILE *progFile)
{
  ...
  initialise_function_program_variables(p) ; 
...
/* Define starting values for function related variables. */ 
void initialise_function_program_variables(Prog *p) 
{
  p->totalFuncs = 0 ; 
  
  p->tableSize = 0 ; 
  
  p->mainStart = STARTING_PLACE ; 
    
  p->inLoop = False ; 
  
  p->loopStart = STARTING_PLACE ;
...
void free_program_memory(Prog **p) 
{
  ...
  /* Free all memory allocated to the function table. */   
  free_function_table(pFree) ; 
...
/* If it exists, free the function hash table. */ 
void free_function_table(Prog *p)
{
  unsigned int i ; 
  
  if(p->funcTable != NULL){
    for(i = 0 ; i < p->totalFuncs ; i++){
      if(p->funcTable[i] != NULL){
        free_function_table_element(p->funcTable[i]) ; 
...
/* Used to free each individual element of hash table. */ 
void free_function_table_element(Func *table)
{
  unsigned int i ; 

  free(table->name) ; 

  for(i = 0 ; i < table->totalInputs ; i++){
    free(table->inputs[i]->name) ; 
    
    table->inputs[i]->name = NULL ; 
    
    free(table->inputs[i]->temp) ; 
    
    table->inputs[i]->temp = NULL ; 
