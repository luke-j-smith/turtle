...
unsigned int varPlace)
{
  unsigned int polishLength = MAX_POLISH_LEN ; 
...
void polish(Prog *p)
{
  ...
  if(str_same(p->program[p->place], EXPRESSION_TERM_CHAR)){
    return ; 
  }
...
/* Draw the new move in an SDL window. */ 
...
/* Draw a forward (ie, FD) move on the screen in white. */
void display_move_on_sdl(SDL_Simplewin sw, Prog *p) 
{
  ...
  do{
    /* Delay SDL update for a short period of time. */
    SDL_Delay(MILLISECONDDELAY);

    /* Draw the line is set colour - currently white. */
    Neill_SDL_SetDrawColour(&sw, SDL_R, SDL_G, SDL_B) ;
    
    /* Draw straight line from old (x, y) to new (x, y). */
...
/* Used to keep SDL Turtle display open until not wanted. */
void keep_sdl_until_finished(SDL_Simplewin sw) 
{
  do{
    ...
    /* Check to see if anyone has killed the SDL window. */
    Neill_SDL_Events(&sw);
